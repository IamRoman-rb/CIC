...
const model = {
    ...
    allWithExtras: () => model.all().map(element => Object.assign(element, {
        ...
        courses: element.courses.map(course => courseModel.oneWithExtras(course)),
        subjects: element.subjects.map(subject => subjectModel.oneWithExtras(subject))
    })), 
    ...
    search: (field, value) => model.allWithExtras().find(element => element[field].toLowerCase().includes(value.toLowerCase())),
    ...
    filterBySubject: subject => model.allWithExtras().filter(teacher => teacher.subjects.filter(element => element.id === subject).length != 0),
}
